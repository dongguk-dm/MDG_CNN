...
class PerceptualLoss():
	...
	def contentFunc(self):
		conv1   = models.resnet34(pretrained=True).conv1
		bn1     = models.resnet34(pretrained=True).bn1
		relu    = models.resnet34(pretrained=True).relu
		maxpool = models.resnet34(pretrained=True).maxpool
		layer1  = models.resnet34(pretrained=True).layer1
		...
		model.add_module(str(0), conv1)
		model.add_module(str(1), bn1)
		model.add_module(str(2), relu)
		model.add_module(str(3), maxpool)
		model.add_module(str(4), layer1)
...
def init_loss(opt, tensor):
	...
	print("Model [%s] recognized." % opt.model)
	...
	print("GAN [%s] recognized." % opt.gan_type)
